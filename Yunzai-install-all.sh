...
#安装pip3.8
curl -sL https://gitee.com/Ganyu256/Yunzai/raw/main/get-pip.py | python3.8
echo pip3.8升级成功
echo
...
#!/bin/bash
#termux-ubuntu自动配置Yunzai-Bot音频转码“ffmpeg相关依赖”，其他linux系统自测！x86平台自测！
#由于ffmpeg从Git拉取，众所周知Git网速极其稳定（bushi）。如果出现拉取特别慢，可以ctrl+c来终止。在本项目内输入./app/ffmpeg-install.sh 来单独安装ffmpeg！
git clone https://gitee.com/Ganyu256/ffmpeg-install ~/ffmpeg-install
...
echo
echo 部署系统编译环境：
apt install -y automake autoconf libtool gcc gcc-
if [ $? -ne 0 ]; then
    echo "失败了！"
else
    echo "编译环境配置成功！"
